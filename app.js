...
if (confirmWithdrawBtn) {
    confirmWithdrawBtn.addEventListener('click', () => {
        ...
        // 1. Проверяем, введено ли число больше или равное минимальному (0.5)
        if (isNaN(amount) || amount < 0.5) {
            alert('Минимальная сумма для вывода: 0.5!');
            return;
        }

        // 2. Берем текущий баланс игрока (если его нет, берем 0 или дефолтное значение)
        let currentBalance = parseFloat(localStorage.getItem('userBalance')) || 0;

        // 3. Проверяем, достаточно ли средств на балансе
        if (amount > currentBalance) {
            alert('Недостаточно средств на балансе!');
            return;
        }

        // 4. Списываем ровно ту сумму, которую вывели
        currentBalance -= amount;
        
        // Сохраняем обновленный баланс
        localStorage.setItem('userBalance', currentBalance);

        // Обновляем баланс на экране во всех элементах с классом .user-balance
